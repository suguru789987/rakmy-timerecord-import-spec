...
- **選択中サービス名を左上に明示**：`連携元サービス ▶ [ロゴ] 名称` バー（`#saas-wiz-svc-bar`）。
- **「取り込み設定」を「取り込むデータを順番に」カード右上へ移設**（フッター btn-group から移動）。
- **一時保存の活性条件**：未アップロードでは「一時保存」「一時保存して中断」を押下不可。判定は `wizHasAnyUpload`（ウィザードのアップロード有無）を正本に一本化し `syncWizSaveButtons()` が両ボタンを同期。テンプレ取込（s-tpl1）にも同適用。
- 反映：README §2.3/§2.4/§2.5、詳細は `CHANGELOG.md` / `JUDGMENT_LOG.md`（2026-07-13）。
